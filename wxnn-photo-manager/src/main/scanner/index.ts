...
import { detectSceneCategory, type SceneCategory, type SceneTime } from '../utils/scene-category'
import { analyzeSceneBrightness } from '../utils/scene-brightness'
// C-3：统一媒体扩展名常量（取代本文件内联定义）
import { IMAGE_EXTENSIONS, VIDEO_EXTENSIONS, getAlbumTypeFromDirName } from '../utils/media-constants'
...
// C-G10：scanner 日志统一走 logger（取代 console.*），便于日志收集与诊断
...
const GAME_SIGNATURES_LOWER = new Set(
  GAME_SIGNATURES.map((s) => s.toLowerCase())
)
...
const MEDIA_FOLDER_SIGNATURES_LOWER = new Set(
  MEDIA_FOLDER_SIGNATURES.map((s) => s.toLowerCase())
)
...
// 注：extractUidFromPath / extractAlbumTypeFromPath / extractMediaSourceFromPath
// 已抽取到 ./path-classifier.ts，便于单元测试（无 electron/sharp/ffmpeg 依赖）
...
export class ScannerManager extends EventEmitter {
  ...
  // A7：DatabaseManager 引用，用于复用统一的迁移接口（isMigrationApplied/markMigrationApplied）
  // 为 null 时降级为直接查询 schema_migrations 表（保持向后兼容）
  private dbManager: DatabaseManager | null = null
  // C-G6：缓存 saveBatchToDatabase 的 prepared statement，避免每个批次都重新 prepare
  // 生命周期：db 变化时（setDatabase 调用）置空，下次调用 saveBatchToDatabase 时重新 prepare
  private insertStmtCache: Database.Statement | null = null
  ...
  setDatabase(db: Database.Database): void {
    this.db = db
    // 切换数据库时清空缓存，避免使用旧连接的 statement
    this.insertStmtCache = null
  }

  /**
   * A7：注入 DatabaseManager 实例，使 scanner 可调用统一迁移接口
   * 可选注入，未注入时降级为直接查询 schema_migrations 表
   */
   ...
   */
  private async cachedRealpath(dir: string): Promise<string> {
    const cached = this.realpathCache.get(dir)
    if (cached) return cached
    const real = await fsp.realpath(dir)
    this.realpathCache.set(dir, real)
    return real
  }
   ...
   */
  private repairLegacyData(): void {
    if (!this.db) return

    // A7：优先使用 DatabaseManager 统一迁移接口；未注入时降级为直接查询
    if (this.dbManager) {
      if (this.dbManager.isMigrationApplied('repair_legacy_data_v1')) {
        return
      }
    } else {
      // P0-A2 兼容兜底：直接查询 schema_migrations 表
      try {
        const applied = this.db.prepare('SELECT 1 FROM schema_migrations WHERE name = ?').get('repair_legacy_data_v1')
        if (applied) {
          return
        }
      } catch {
    ...
    try {
      ...
      const nonGameFiles = this.db.prepare(
        "SELECT id FROM media_files WHERE LOWER(file_path) NOT LIKE '%infinitynikki%'"
      ).all() as Array<{ id: number }>

      if (nonGameFiles.length > 0) {
        const deleteStmt = this.db.prepare('DELETE FROM media_files WHERE id = ?')
        const deleteMany = this.db.transaction((ids: number[]) => {
          for (const id of ids) deleteStmt.run(id)
        })
        deleteMany(nonGameFiles.map(f => f.id))
        logger.info(`[Scanner] repairLegacyData: 删除 ${nonGameFiles.length} 条误扫描的非游戏目录文件`)
      }
      ...
      const legacyFiles = this.db.prepare(
      ...
      if (legacyFiles.length > 0) {
        ...
        const updateMany = this.db.transaction((files: Array<{ id: number; file_path: string }>) => {
        ...
        logger.info(`[Scanner] repairLegacyData: 更新 ${legacyFiles.length} 条旧记录的 media_source/account_uid/album_type 字段`)
      }

      // A7：标记迁移已完成，优先使用 DatabaseManager 统一接口
      try {
        if (this.dbManager) {
          this.dbManager.markMigrationApplied('repair_legacy_data_v1')
        } else {
          this.db.prepare(
        ...
        logger.warn('[Scanner] repairLegacyData: 标记迁移状态失败，下次将重复执行:', markErr instanceof Error ? markErr.message : String(markErr))
      }
    } catch (err) {
      // 修复失败不阻塞扫描，仅记录错误
      logger.error('[Scanner] repairLegacyData 失败:', err instanceof Error ? err.message : String(err))
    }
  }
  ...
  async startScan(options?: ScanOptions): Promise<{ success: boolean; message: string; filesFound?: number }> {
    ...
    this.progress = { scanned: 0, found: 0, currentPath: '', status: 'running' }
    ...
    const scanId = this.insertScanHistory(scanType)

    // 修复旧数据：删除误扫描的非游戏目录文件 + 重新计算 media_source='unknown' 记录的字段
    ...
    try {
      ...
      if (options?.fullScan && !options?.path) {
        const mediaDirs = await this.findMediaDirectories(options?.customKnownPaths)
        if (mediaDirs.length === 0) {
          this.progress.status = 'failed'
          this.updateScanHistory(scanId, 'failed', 0, 0)
        ...
        for (const dir of mediaDirs) {
          ...
          this.progress.currentPath = dir
          const { files, savedCount } = await this.scanDirectory(dir, options?.incremental)
        ...
        this.updateScanHistory(scanId, 'completed', totalFiles, totalSaved)
      ...
      if (!targetPath) {
        ...
        const gameDirs = await this.findMediaDirectories(options?.customKnownPaths)
      ...
      const { files, savedCount } = await this.scanDirectory(targetPath, options?.incremental)

      // T02：扫描完成后执行文件完整性校验
      await this.checkMissingFiles()

      this.progress.status = 'completed'
      this.updateScanHistory(scanId, 'completed', files.length, savedCount)
      ...
      this.progress.status = 'failed'
      this.updateScanHistory(scanId, 'failed', this.progress.found, 0)
      return {
        success: false,
  ...
  private insertScanHistory(scanType: string): number {
    if (!this.db) return 0
  ...
  private updateScanHistory(id: number, status: string, filesFound: number, filesNew: number): void {
    if (!this.db) return
    this.db.prepare(
  ...
  private async checkMissingFiles(): Promise<void> {
    ...
    try {
      ...
      const updateFound = this.db.prepare(
        'UPDATE media_files SET missing_count = 0, is_missing = 0 WHERE id = ?'
      )
      const updateAll = this.db.transaction((missingIds: number[], foundIds: number[]) => {
      ...
      if (missingIds.length > 0) {
        logger.info(`[Scanner] 完整性校验：${missingIds.length} 个文件缺失，${foundIds.length} 个文件存在`)
      }
    } catch (error) {
      // logger 对 Error 对象会用 JSON.stringify 丢失 stack，转 message 保留可读信息
      logger.error('[Scanner] 完整性校验失败:', error instanceof Error ? error.message : String(error))
    }
  }
   ...
   */
  private async findMediaDirectories(_customPaths?: string[]): Promise<string[]> {
    const found = new Set<string>()

    // 1. 快速路径：Steam + Epic 注册表查询
    for (const p of await this.findSteamGamePaths()) found.add(p)
    for (const p of await this.findEpicGamePaths()) found.add(p)

    // 2. 快速路径：用户数据目录（AppData 下的游戏数据）
    // SYSTEM_DIRS_TO_SKIP 跳过 appdata，全盘签名搜索无法进入，必须显式检查
    for (const p of await this.getUserDataPaths()) found.add(p)

    // P1-A3：快速路径早退——Steam/Epic/用户数据目录已找到结果时跳过全盘签名搜索
    // 全盘搜索遍历所有盘符 depth=8，单盘 30s+，快速路径通常已覆盖常见安装位置
    if (found.size > 0) {
      logger.info(`[Scanner] 快速路径已找到 ${found.size} 个目录，跳过全盘签名搜索`)
    } else {
    ...
    const driveTasks = drives.map((drive) => async () => {
      ...
      const mediaDirs = await this.searchByMediaFolderSignatureShallow(drive, 8)
      if (mediaDirs.length > 0) {
        for (const d of mediaDirs) found.add(d)
        return
      }
      // 媒体文件夹没找到，搜索游戏 exe
      const gameDir = await this.searchBySignatureShallow(drive, 8)
    ...
    const filtered = Array.from(found).filter(p =>
      p.toLowerCase().includes('infinitynikki')
    )
    logger.info(`[Scanner] findMediaDirectories 搜索结果: ${filtered.length} 个目录（过滤前 ${found.size} 个）`)
    for (const d of filtered) logger.info(`[Scanner]   - ${d}`)
   ...
   */
  private async searchBySignature<T>(
    rootPath: string,
    maxDepth: number,
    ...
    const search = async (dir: string, depth: number): Promise<void> => {
      if (this.shouldStop || stopped || depth >= maxDepth) return
      try {
        const realDir = await this.cachedRealpath(dir)
        if (visitedRealPaths.has(realDir)) return
        visitedRealPaths.add(realDir)
        const entries = await fsp.readdir(dir, { withFileTypes: true })
        for (const entry of entries) {
          if (this.shouldStop || stopped) return
        ...
        logger.warn(`[Scanner] 跳过目录: ${dir}`, err instanceof Error ? err.message : String(err))
      }
    }
   ...
   */
  private async searchBySignatureShallow(rootPath: string, maxDepth: number): Promise<string | null> {
    const results = await this.searchBySignature<string>(rootPath, maxDepth, (entry, dir) => {
   ...
   * @param maxDepth 最大搜索深度
   */
  private async searchByMediaFolderSignatureShallow(rootPath: string, maxDepth: number): Promise<string[]> {
    const results = await this.searchBySignature<string>(rootPath, maxDepth, (entry, dir) => {
   ...
   * 返回所有匹配路径（可能为空数组）
   */
  private async findSteamGamePaths(): Promise<string[]> {
    if (process.platform !== 'win32') return []

    const steamInstallPath = await this.querySteamInstallPath()
   ...
   */
  private async findEpicGamePaths(): Promise<string[]> {
    if (process.platform !== 'win32') return []
    ...
    for (const { key, value } of regKeys) {
      try {
        const result = await new Promise<string | null>((resolve) => {
          execFile('reg', ['query', key, '/v', value], { timeout: 3000, windowsHide: true }, (err, stdout) => {
   ...
   */
  private async querySteamInstallPath(): Promise<string | null> {
    const regKeys = [
  ...
  private async scanDirectory(rootPath: string, incremental?: boolean): Promise<{ files: MediaFile[]; savedCount: number }> {
    const files: MediaFile[] = []
    let savedCount = 0

    // 增量扫描：获取已索引的文件路径和修改时间
    const existingFiles = new Map<string, string>()
    if (incremental && this.db) {
      const rows = this.db.prepare('SELECT file_path, modified_at FROM media_files').all() as Array<{ file_path: string; modified_at: string }>
    ...
    const flushBatch = async () => {
      ...
      batchBuffer = []
      savedCount += await this.saveBatchToDatabase(batch)
    }
    ...
    const pendingVideos: Array<{ fullPath: string; ext: string; entryName: string; stats: fs.Stats }> = []
    ...
    const scan = async (dir: string, depth: number) => {
      ...
      if (depth > MAX_SCAN_DEPTH) {
        logger.warn(`[Scanner] 超过最大扫描深度 ${MAX_SCAN_DEPTH}，跳过: ${dir}`)
        return
      }
      ...
      try {
        ...
        if (visitedRealPaths.has(realDir)) {
          logger.warn(`[Scanner] 检测到符号链接环，跳过: ${dir} -> ${realDir}`)
          return
        }
        visitedRealPaths.add(realDir)
          ...
          if (entry.isDirectory()) {
            ...
            if (IMAGE_EXTENSIONS.has(ext) || VIDEO_EXTENSIONS.has(ext)) {
              ...
              if (incremental) {
                ...
                if (existingMtime && Math.abs(new Date(existingMtime).getTime() - stats.mtime.getTime()) <= 1000) {
                  continue
                }
              }
    ...
    const videoTasks = pendingVideos.map(({ fullPath, ext, entryName, stats }) => async () => {
      ...
      try {
        ...
        logger.error(`[Scanner] 跳过视频元数据读取失败: ${fullPath}`, err instanceof Error ? err.message : String(err))
      }
      await addFile({
        file_path: fullPath,
        file_name: entryName,
  ...
  private async saveBatchToDatabase(batch: MediaFile[]): Promise<number> {
    if (!this.db || batch.length === 0) {
      if (!this.db) logger.info(`[Scanner] 数据库未连接，跳过保存 ${batch.length} 个文件`)
      return 0
    }

    // C-G6：复用缓存的 prepared statement，避免每批次重新 prepare
    if (!this.insertStmtCache) {
      this.insertStmtCache = this.db.prepare(`
    ...
    const insertStmt = this.insertStmtCache

    const insertMany = this.db.transaction((mediaFiles: MediaFile[]) => {
    ...
    insertMany(batch)
    logger.info(`[Scanner] 流式写入批次：${batch.length} 个文件`)
    return batch.length
  }
}
