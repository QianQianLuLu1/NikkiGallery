...
describe('MediaRepository', () => {
  ...
  // ==========================================================================
  describe('单行更新', () => {
    it('updateRating 正常更新评分', () => {
      const id = insertMedia(db, { rating: 0 })
      repo.updateRating(id, 5)
    ...
    it('updateRating mediaId 不存在时不抛错', () => {
      expect(() => repo.updateRating(99999, 5)).not.toThrow()
    })

    it('updateFavorite true → 1', () => {
    ...
    it('updateFavorite false → 0', () => {
      const id = insertMedia(db, { is_favorite: 1 })
      repo.updateFavorite(id, false)
      const row = db.prepare('SELECT is_favorite FROM media_files WHERE id = ?').get(id) as {
        is_favorite: number
      }
    ...
    it('updateTags 单元素与多元素', () => {
      const id = insertMedia(db)
      repo.updateTags(id, ['风景'])
      expect(
        (db.prepare('SELECT tags FROM media_files WHERE id = ?').get(id) as { tags: string }).tags
      ).toBe('["风景"]')

      repo.updateTags(id, ['风景', '人物', '夜景'])
      expect(
        (db.prepare('SELECT tags FROM media_files WHERE id = ?').get(id) as { tags: string }).tags
      ).toBe('["风景","人物","夜景"]')
    })

    it('updateTags 含中文/特殊字符', () => {
      const id = insertMedia(db)
      repo.updateTags(id, ['暖暖"套装"', '<script>'])
      const row = db.prepare('SELECT tags FROM media_files WHERE id = ?').get(id) as {
        tags: string
      }
    ...
    it('updateCategory 正常赋值', () => {
      const catId = insertCategory(db)
      const id = insertMedia(db)
      repo.updateCategory(id, catId)
      const row = db.prepare('SELECT category_id FROM media_files WHERE id = ?').get(id) as {
        category_id: number | null
      }
    ...
    it('updateSceneTime 更新场景时段', () => {
      const id = insertMedia(db, { scene_time: 'unknown' })
      repo.updateSceneTime(id, 'day')
      expect(
        (
  ...
  describe('批量删除/恢复', () => {
    ...
    it('softDeleteBatch 空数组 no-op', () => {
      expect(() => repo.softDeleteBatch([])).not.toThrow()
    })

    it('softDeleteBatch 单条软删除', () => {
      const id = insertMedia(db, { is_deleted: 0 })
      repo.softDeleteBatch([id])
      const row = db.prepare('SELECT is_deleted, deleted_at FROM media_files WHERE id = ?').get(id)
      expect(row?.is_deleted).toBe(1)
      expect(row?.deleted_at).not.toBeNull()
    })

    it('softDeleteBatch 多条批量软删除', () => {
      ...
      repo.softDeleteBatch([id1, id2, id3])
      const rows = db
    ...
    it('restoreBatch 空数组 no-op', () => {
      expect(() => repo.restoreBatch([])).not.toThrow()
    })

    it('restoreBatch 单条恢复', () => {
      const id = insertMedia(db, { is_deleted: 1, deleted_at: '2024-01-01 10:00:00' })
      repo.restoreBatch([id])
      const row = db.prepare('SELECT is_deleted, deleted_at FROM media_files WHERE id = ?').get(id)
      expect(row?.is_deleted).toBe(0)
      expect(row?.deleted_at).toBeNull()
    })

    it('restoreBatch 多条批量恢复', () => {
    ...
    it('restoreBatch 非软删除 id 幂等', () => {
      const id = insertMedia(db, { is_deleted: 0 })
      expect(() => repo.restoreBatch([id])).not.toThrow()
    ...
    it('softDeleteForPermanentDelete 空数组 no-op', () => {
      expect(() => repo.softDeleteForPermanentDelete([])).not.toThrow()
    })

    it('softDeleteForPermanentDelete 多条标记软删除', () => {
      const id1 = insertMedia(db, { file_path: '/t/1.jpg' })
      const id2 = insertMedia(db, { file_path: '/t/2.jpg' })
      repo.softDeleteForPermanentDelete([id1, id2])
      const rows = db
        .prepare('SELECT COUNT(*) as count FROM media_files WHERE is_deleted = 1')
        .get() as { count: number }
    ...
    it('hardDeleteBatch 空数组 no-op', () => {
      expect(() => repo.hardDeleteBatch([])).not.toThrow()
    })

    it('hardDeleteBatch 多条批量删除', () => {
      const id1 = insertMedia(db, { file_path: '/t/1.jpg' })
      const id2 = insertMedia(db, { file_path: '/t/2.jpg' })
      const id3 = insertMedia(db, { file_path: '/t/3.jpg' })
      repo.hardDeleteBatch([id1, id2, id3])
    ...
    it('hardDeleteBatch 含不存在的 id 不抛错', () => {
      const id = insertMedia(db)
      expect(() => repo.hardDeleteBatch([id, 99999, 88888])).not.toThrow()
      const row = db.prepare('SELECT id FROM media_files WHERE id = ?').get(id)
      expect(row).toBeUndefined()
    })

    it('cleanupMissingRecords 返回删除条数', () => {
      ...
      const deleted = repo.cleanupMissingRecords()
      expect(deleted).toBe(2)
    })

    it('cleanupMissingRecords 无缺失时返回 0', () => {
      insertMedia(db, { is_missing: 0 })
      const deleted = repo.cleanupMissingRecords()
      expect(deleted).toBe(0)
    })

    it('removeMissingRecord 删除 missing 记录返回 true', () => {
      const id = insertMedia(db, { is_missing: 1 })
      expect(repo.removeMissingRecord(id)).toBe(true)
      const row = db.prepare('SELECT id FROM media_files WHERE id = ?').get(id)
      expect(row).toBeUndefined()
    })

    it('removeMissingRecord 非 missing 记录返回 false', () => {
      const id = insertMedia(db, { is_missing: 0 })
      expect(repo.removeMissingRecord(id)).toBe(false)
      const row = db.prepare('SELECT id FROM media_files WHERE id = ?').get(id)
  ...
  describe('查询接口', () => {
    ...
    it('setMediaCountCache 写入后 listMedia 间接读取', () => {
      repo.setMediaCountCache(42)
      const result = repo.listMedia({ page: 0, pageSize: 10 })
      expect(result.total).toBe(42)
    })

    it('refreshMediaCountCache 刷新为当前未软删除记录数', () => {
      insertMedia(db, { file_path: '/t/1.jpg', is_deleted: 0 })
      insertMedia(db, { file_path: '/t/2.jpg', is_deleted: 0 })
      insertMedia(db, { file_path: '/t/3.jpg', is_deleted: 1 })
      repo.refreshMediaCountCache()
      const result = repo.listMedia({ page: 0, pageSize: 10 })
  ...
  // ==========================================================================
  describe('分类', () => {
    it('createCategory 返回新 id', () => {
      const id = repo.createCategory('新分类', 'icon', '#ff0000', null)
      expect(typeof id).toBe('number')
      expect(id).toBeGreaterThan(0)
    })

    it('createCategory sort_order 自增', () => {
      const id1 = repo.createCategory('分类1', 'icon', '#000', null)
      const id2 = repo.createCategory('分类2', 'icon', '#000', null)
    ...
    it('createCategory parent_id 设置', () => {
      const parentId = repo.createCategory('父分类', 'icon', '#000', null)
      const childId = repo.createCategory('子分类', 'icon', '#000', parentId)
    ...
    it('updateCategoryFields 空 fields 跳过', () => {
      const id = repo.createCategory('测试', 'icon', '#000', null)
      expect(() => repo.updateCategoryFields(id, [], [])).not.toThrow()
    })

    it('updateCategoryFields 单字段更新', () => {
      const id = repo.createCategory('测试', 'icon', '#000', null)
      repo.updateCategoryFields(id, ['name = ?'], ['新名字'])
    ...
    it('updateCategoryFields 多字段更新', () => {
      const id = repo.createCategory('测试', 'icon', '#000', null)
      repo.updateCategoryFields(id, ['name = ?', 'color = ?'], ['新名字', '#ffffff'])
    ...
    it('deleteCategoryCascade 级联置 NULL media_files.category_id', () => {
      const catId = repo.createCategory('测试', 'icon', '#000', null)
      const mediaId = insertMedia(db, { category_id: catId })
      repo.deleteCategoryCascade(catId)
    ...
    it('deleteCategoryCascade 非系统分类正常删除', () => {
    ...
    it('reorderCategories 批量更新 sort_order + parent_id', () => {
    ...
    it('reorderCategories 空数组 no-op', () => {
      expect(() => repo.reorderCategories([])).not.toThrow()
    })
  ...
  describe('角色档案', () => {
    ...
    it('addProfile 新增后 listProfiles 能查到', () => {
      repo.addProfile('new-uid', '新角色', null)
      const result = repo.listProfiles()
      expect(result.find((p) => p.uid === 'new-uid')).toBeDefined()
      expect(result.find((p) => p.uid === 'new-uid')?.nickname).toBe('新角色')
    })

    it('updateProfileFields 空 sets 跳过', () => {
      insertProfile(db, { uid: 'u1', nickname: '原名' })
      expect(() => repo.updateProfileFields('u1', [], [])).not.toThrow()
      const result = repo.listProfiles()
      expect(result.find((p) => p.uid === 'u1')?.nickname).toBe('原名')
    })

    it('updateProfileFields 单字段更新', () => {
      insertProfile(db, { uid: 'u1', nickname: '原名' })
      repo.updateProfileFields('u1', ['nickname = ?'], ['新名'])
      const result = repo.listProfiles()
      expect(result.find((p) => p.uid === 'u1')?.nickname).toBe('新名')
    })

    it('updateProfileFields 多字段更新', () => {
    ...
    it('deleteProfileAndReassign 将 media_files 迁移到 default + 删除档案', () => {
      ...
      repo.deleteProfileAndReassign('u1')
    ...
    it('touchProfileActive 更新 last_active_at', () => {
      insertProfile(db, { uid: 'u1', last_active_at: null })
      repo.touchProfileActive('u1')
      const row = db
    ...
    it('transferFilesToProfile 空数组 no-op', () => {
      expect(() => repo.transferFilesToProfile([], 'target')).not.toThrow()
    })

    it('transferFilesToProfile 批量更新 account_uid', () => {
      ...
      repo.transferFilesToProfile([id1, id2], 'target')
      const rows = db
  ...
  describe('transaction', () => {
    ...
    it('事务支持返回值', () => {
      const result = repo.transaction(() => 42)
