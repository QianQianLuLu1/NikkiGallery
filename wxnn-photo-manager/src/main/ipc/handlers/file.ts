...
export function registerFileHandlers(ctx: HandlerContext): void {
  ...
  ipcMain.handle(
    'file:batchRename',
    async (_, operations: { oldPath: string; newName: string }[]) => {
  ...
  ipcMain.handle(
    'file:export',
    async (_, filePaths: string[], targetDir: string, options?: ExportOptions) => {
  ...
  ipcMain.handle(
    'file:saveAs',
    async (_, filePath: string, targetDir: string, newName?: string) => {
