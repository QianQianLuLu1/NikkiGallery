...
export function registerVideoHandlers(ctx: HandlerContext): void {
  ...
  ipcMain.handle(
    'video:captureFrame',
    async (_, filePath: string, currentTime: number, targetDir?: string) => {
      // A-S9：参数校验
      const v1 = validateFilePath(filePath)
      if (!v1.valid) return { success: false, message: v1.message }
  ...
  ipcMain.handle(
    'video:trim',
    async (_, filePath: string, startTime: number, endTime: number, targetDir: string) => {
      const v1 = validateFilePath(filePath)
      if (!v1.valid) return { success: false, message: v1.message }
      const v2 = validateFilePath(targetDir)
      if (!v2.valid) return { success: false, message: v2.message }
  ...
  ipcMain.handle(
    'video:changeSpeed',
    async (_, filePath: string, speed: number, targetDir: string) => {
      const v1 = validateFilePath(filePath)
      if (!v1.valid) return { success: false, message: v1.message }
      const v2 = validateFilePath(targetDir)
      if (!v2.valid) return { success: false, message: v2.message }
