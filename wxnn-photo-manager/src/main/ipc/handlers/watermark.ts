...
// A-S9：IPC 参数校验工具
import { validateIntRange, validateFilePathArray, validateFilePath, validateNonSensitivePath } from '../../utils/ipc-validate'
// 水印配置类型
import type { WatermarkConfig } from '../../types/file'

export function registerWatermarkHandlers(ctx: HandlerContext): void {
  // 水印
  ipcMain.handle('watermark:apply', async (_, config: unknown, filePaths: string[], targetDir: string) => {
    try {
