...
export function registerCrashHandlers(ctx: HandlerContext): void {
  ...
  ipcMain.handle('crash:list', async () => {
    try {
      ...
      return { success: false, crashes: [], message: String(error) }
    }
  })
  ...
  // 打开崩溃目录（系统资源管理器）
  ipcMain.handle('crash:openDirectory', async () => {
    return openCrashDirectory()
  })
  ...
  ipcMain.handle('crash:getDir', async () => getCrashDirectory())
}
