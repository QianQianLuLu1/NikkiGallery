...
export const ShareCodeDecoderDialog: React.FC<ShareCodeDecoderDialogProps> = ({ open, onClose }) => {
  ...
  const handleDecode = useCallback(async () => {
    ...
    try {
      ...
      setResult({ success: false, message: err instanceof Error ? err.message : t('common.shareCodeDecoder.unknownError') })
    } finally {
      setLoading(false)
    }
  }, [input, tab, t])
    ...
    <BaseDialog open={open} onClose={onClose} size="lg" ariaLabelledby="share-code-decoder-title">
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 id="share-code-decoder-title" className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>
        ...
        {/* Tab 选择器 */}
        <div className="flex gap-1 mb-4 p-1 rounded-xl" style={{ background: 'var(--bg-secondary)' }}>
        ...
        >
          {loading ? t('common.shareCodeDecoder.decoding') : tab === 'mediaEncrypt' ? t('common.shareCodeDecoder.encryptButton') : t('common.shareCodeDecoder.decodeButton')}
        </button>

        {/* 结果区 */}
        {result && (
          <div className="mt-4 p-4 rounded-xl text-sm" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--divider)' }}>
            {result.success ? (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <span className="w-2 h-2 rounded-full bg-green-500" />
                  <span className="font-medium" style={{ color: 'var(--text-primary)' }}>
                    {tab === 'mediaEncrypt' ? t('common.shareCodeDecoder.encryptSuccess') : t('common.shareCodeDecoder.decodeSuccess')}
                  </span>
                </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <div className="font-medium text-red-500 mb-1">{t('common.shareCodeDecoder.operationFailed')}</div>
...
const ResultDisplay: React.FC<{ tab: DecoderTab; data: unknown }> = ({ tab, data }) => {
  ...
  const Row: React.FC<{ label: string; value: string }> = ({ label, value }) => (
    <div className="flex items-baseline gap-3 py-1.5 border-b last:border-b-0" style={{ borderColor: 'var(--border-color, rgba(0,0,0,0.04))' }}>
      <span className="flex-shrink-0 w-24 text-xs" style={{ color: 'var(--text-tertiary)' }}>{label}</span>
      <span className="flex-1 break-all font-mono text-xs" style={{ color: 'var(--text-primary)' }}>{value}</span>
    </div>
  )
  ...
  if (tab === 'clothDiy') {
    return (
      <div>
        {d.timestamp != null && <Row label={t('common.shareCodeDecoder.result.timestamp')} value={String(d.timestamp)} />}
        {d.uidBytes != null && <Row label="UID (Hex)" value={String(d.uidBytes)} />}
        {d.networkData != null ? (
          <Row label={t('common.shareCodeDecoder.result.networkData')} value={String(d.networkData).slice(0, 500) + (String(d.networkData).length > 500 ? '...' : '')} />
        ) : (
          <div className="text-xs py-2" style={{ color: 'var(--text-tertiary)' }}>{t('common.shareCodeDecoder.result.networkUnavailable')}</div>
        )}
      </div>
    )
  }
  ...
  if (tab === 'homeBuild') {
    return (
      <div>
        {d.server != null && <Row label={t('common.shareCodeDecoder.result.serverId')} value={String(d.server)} />}
        {d.networkData != null ? (
          <Row label={t('common.shareCodeDecoder.result.networkData')} value={String(d.networkData).slice(0, 500) + (String(d.networkData).length > 500 ? '...' : '')} />
        ) : (
          <div className="text-xs py-2" style={{ color: 'var(--text-tertiary)' }}>{t('common.shareCodeDecoder.result.networkUnavailable')}</div>
  ...
  return <Row label={t('common.shareCodeDecoder.result.encryptedResult')} value={dataStr.slice(0, 500) + (dataStr.length > 500 ? '...' : '')} />
}
