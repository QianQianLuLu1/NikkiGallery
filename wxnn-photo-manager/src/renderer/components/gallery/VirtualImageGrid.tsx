...
const VirtualImageGridComponent: React.FC<VirtualImageGridProps> = (props) => {
  const { items, selectedIds, onSelect, onHover, onContextMenu, onEdit, onFavorite, onDelete } = props
  ...
  return (
    <div
      ...
      aria-label="图片网格"
    >
      <div ref={containerRef} className="h-full overflow-y-auto" onScroll={triggerHighQualityUpgrade}>
        <div style={{ height: totalHeight, position: 'relative' }}>
          ...
          >
            {visibleItems.map(({ item, index }) => {
              ...
              const thumbUrl = useHighQuality || !lowUrl ? standardUrl : toFileUrl(lowUrl)
              ...
              return (
                <div
                  ...
                  onMouseEnter={() => { setHoveredId(file.id); onHover(file.id) }}
                ...
                >
                  <div className="w-full h-full flex items-center justify-center relative overflow-hidden rounded-xl" style={{ background: 'var(--bg-tertiary)' }}>
                    ...
                    {hoveredId === file.id && (
                      <>
                        <div className="absolute inset-0 flex flex-col justify-end p-3" style={{ background: 'linear-gradient(transparent 50%, var(--overlay-bg-strong))' }}>
                          {/* P2 修复：原 truncate 单行截断长文件名，改用 ellipsis-2 两行省略显示更多文字 */}
                          <p className="text-white text-xs font-medium ellipsis-2">{file.file_name}</p>
                          <p className="text-white/70 text-xs">{formatFileSize(file.file_size)}</p>
                        </div>
                        <MediaCardActions file={file} onEdit={onEdit} onFavorite={onFavorite} onDelete={onDelete} variant="hover" />
                      </>
                    )}

                    {/* U-G16：触屏设备始终显示快捷操作按钮（无 hover 事件），desktop 仍走 hover 路径 */}
                    {isTouchDevice && hoveredId !== file.id && (
                      <MediaCardActions file={file} onEdit={onEdit} onFavorite={onFavorite} onDelete={onDelete} variant="touch" />
                    )}

                    {/* U-G16：触屏设备显示文件名条以便识别（避开右下角的时长徽章） */}
                    {isTouchDevice && hoveredId !== file.id && (
                      <div className="absolute inset-x-0 bottom-0 left-0 right-8 p-2" style={{ background: 'var(--overlay-gradient-bottom)' }}>
                    ...
                    {file.duration && (
                      <div className="absolute bottom-2 right-2 px-1.5 py-0.5 rounded text-xs font-medium text-white" style={{ background: 'var(--overlay-bg-strong)' }}>
