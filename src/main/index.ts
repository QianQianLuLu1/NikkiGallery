...
// 数据库写操作进程拆分：better-sqlite3 同步写通过 DatabaseWorkerBridge 走独立 utilityProcess
import { DatabaseWorkerBridge } from './database/worker/database-worker-bridge'
...
// IPC 安全加固：pathGuard 用于文件操作类 handler 的路径白名单校验
import { pathGuard } from './ipc/validator'
...
class Application {
  ...
  // 数据库写操作进程拆分：database worker 独立 utilityProcess，承接所有 better-sqlite3 写操作
  private databaseWorkerBridge: DatabaseWorkerBridge
  ...
  constructor() {
    ...
    // 数据库写操作进程拆分：bridge 在 dbManager.initialize() 完成后注入到 dbManager
    this.databaseWorkerBridge = new DatabaseWorkerBridge()
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        // 数据库写操作进程拆分：initialize 完成后注入 worker bridge，
        // MediaRepository 通过 getWorkerBridge() 走 worker 进程执行写操作
        this.dbManager.setWorkerBridge(this.databaseWorkerBridge)
      } catch (dbErr) {
  ...
  private async performCleanup(): Promise<void> {
    ...
    // 3.7 数据库写操作进程拆分：dispose database worker 进程
    // 必须在 dbManager.close() 之前完成，确保 worker 中的 WAL checkpoint 落盘
    // 加 1s 超时保护，避免 worker 卡住阻塞主进程退出
    try {
      await Promise.race([
        this.databaseWorkerBridge.dispose(),
        new Promise<void>((resolve) => setTimeout(resolve, 1000))
      ])
    } catch {}
  ...
  private setupIPC(): void {
    ...
    // IPC 安全加固：初始化 pathGuard 的 userData 目录白名单
    // 必须在 app.whenReady() 之后调用，app.getPath('userData') 才可用
    // userHome 已在 PathGuard 构造时初始化，此处补充 userDataDir
    // 覆盖场景：用户主目录下的所有路径默认允许（Documents\InfinityNikki 等）
    // 系统敏感目录（C:\Windows\System32 等）作为黑名单兜底始终拒绝
    pathGuard.initUserDataDir()
  }
  ...
  private notifyMediaUpdated(): void {
    ...
    // 数据库写操作进程拆分：refreshMediaCountCache 改为 async（写操作走 worker 进程）
    // 此处 fire-and-forget：notifyMediaUpdated 是同步 void 方法，不能 await；缓存更新失败不影响主流程
    try {
      const db = this.dbManager.getDatabase()
      if (db) {
        void new MediaRepository(
          db,
          this.dbManager.getWorkerBridge(),
          this.dbManager.getDbPath()
        )
          .refreshMediaCountCache()
          .catch(() => {
